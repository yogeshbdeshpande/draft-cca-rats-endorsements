...
Each CCA Platform Endorsement, be it a Reference Value or an Attestation Verification Key, is associated with a unique CCA Platform identifier known as Implementation ID (see {{Section 4.4.2 of -cca-token}}).
...
CCA provides confidential computing environments, known as Realms, that enable application workloads requiring confidential execution to operate in isolation from the host hypervisor and any other concurrent workload.
CCA allows the initial and run-time state of a Realm to be attested ({{Section 4.8 of -cca-token}}).
